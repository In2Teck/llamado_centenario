...
function goHome() { 
   if ($("#ruby-values").data("has-ticket")) {
    ...
    $("#txt-info").text("YA CUENTAS CON TU BOLETO <p style='font-size:0.5em;'> Tu folio es " + $("#ruby-values").data("folio") + "</p>");
...
function onSearchTicket(data, textStatus, jqXHR) {
 ...
 if (data.won_ticket) {
    ...
    $("#txt-result").text("¡ FELICIDADES !");
    $("#txt-info").text("ENCONTRASTE TU BOLETO <p style='font-size:0.5em;'> Tu folio es " + data.folio + "</p>");
